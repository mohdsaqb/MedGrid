  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      const input: EncounterInput = {
        ...
        doctor_id: doctorId,
        appointment_id: null,
        encounter_date: `${encounterDate}T${encounterTime}:00`,
        diagnosis,
        symptoms,
        notes: notes || null,
      }
      const created = await api.post<Encounter>('/encounters', input)
      navigate(`/encounters/${created.id}`)
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Failed to create encounter')
    } finally {
      setSubmitting(false)
    }
  }
